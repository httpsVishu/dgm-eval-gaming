...
self-improvement, no open-ended exploration) on both SWE-bench and Polyglot. That is
488 individual self-modification attempts, each one a real decision a real agent made
about what to change in its own codebase. This is ground truth and not a guess.
...
did not happen but we needed a controlled experiment to explain why node 114 worked
...
inspection, turned out to be either a broken feature attempt or a legitimate rewrite and
not cheating.

So why did node 114 succeed elsewhere? Our answer as confirmed by the sandbox: it is not
...
But here is a twist that this protection is not absolute. We built a detector specifically
